...
export async function processCharacterDataSync(
	...
	const { characterId, region, realm, characterName } = JobPayloadSchemas[JobTypes.SYNC_CHARACTER_DATA].parse(job.data);
	
	console.log(`Processing character data sync for ${characterName} (${realm}-${region})`);
	...
	try {
		...
		console.error(`Failed to sync character data for ${characterName}:`, error);
		throw error;
	}
}
