...
export async function processGuildSync(
	...
	try {
		const guild = await apiClient.wow.Guild(
			jobData.realm.slug,
			jobData.guild.slug,
		);
		...
		if (!guild.success) {
			throw new Error(
				`Failed to fetch guild data: ${jobData.guild.slug} on realm ${jobData.realm.slug}`,
			);
		}
		...
		console.error(
			`Failed to sync guild data for ${jobData.guild.id}:`,
			error,
		);
		throw error;
	}
