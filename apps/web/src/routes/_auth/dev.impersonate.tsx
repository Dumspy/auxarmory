...
export const Route = createFileRoute('/_auth/dev/impersonate')({
	...
	component: DevImpersonatePage,
})

function DevImpersonatePage() {
	...
	const usersQuery = useQuery({
		queryKey: ['dev', 'impersonate', 'users', listUsersInput],
		queryFn: async () => {
			const result = await authClient.admin.listUsers({
				query: {
					limit: listUsersInput.limit,
					offset: listUsersInput.offset,
					searchField: listUsersInput.searchValue
						? 'email'
						: undefined,
					searchOperator: listUsersInput.searchValue
						? 'contains'
						: undefined,
					searchValue: listUsersInput.searchValue,
				},
			})
	...
	async function handleImpersonate(userId: string) {
		...
		if (result.error) {
			setImpersonateError(result.error.message)
			return
		}
			...
			<div className='space-y-6 p-2 md:p-4'>
				<div>
					...
					<p className='text-muted-foreground'>
						This feature is only available in development.
					</p>
				</div>
			</div>
		)
	...
	return (
		<div className='space-y-6 p-2 md:p-4'>
			<div>
				...
				<p className='text-muted-foreground'>
					Select a user to impersonate. This is only available in
					development.
				</p>
			</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Search users by email and start an impersonation
						session.
					</CardDescription>
				</CardHeader>
