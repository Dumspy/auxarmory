...
schema = schema.replace(/(model\s+\w+\s+\{)([^}]*)(\})/g, (match, start, body, end) => {
  // Skip if already contains createdAt or updatedAt
  if (body.includes('createdAt') || body.includes('updatedAt')) return match;
...
console.log(`✅ Prisma schema generated with createdAt and updatedAt fields in "${outputPath}"`);